...
class AddExperienceForm extends React.Component {
  ...
  onSave = (e) => {
    ...
    axios
      .post('http://localhost:3000/api/student/experience/17', this.state, {headers: {'Content-Type': 'application/json'}})
      .then(res => {
        if (res.status === 200) {
          console.log(res.data.result.insertId);
          this.props.onAddExperience({...this.state, experience_id: res.data.result.insertId, student_id: 17});
