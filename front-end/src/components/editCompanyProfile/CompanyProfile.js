...
class CompanyProfile extends React.Component {
    ...
    onSaveTextArea = () => {
      ...
      axios
        .post('http://localhost:3000/api/student/basic/17', data[0], {headers: {'Content-Type': 'application/json'}})
        .then(res => {
          if (res.status === 200) {
            ...
            <div>
                ...
                <div style={{float: 'left', width: '52.5%', marginLeft: '20px', marginTop: '20px'}} >
                    <div style={{marginBottom: '20px'}}>
                        ...
                        <i className='pencil alternate icon' style={{ float: 'right' }} onClick={this.onAddDescription}></i>
                        <div style={{ marginTop: '10px' }}>
                            {!this.state.showTextFrom && this.state.basicDetail.map(item => {
                            return ( <p style={{ fontSize: '20px' }}>{item.company_description}</p>);
                        })}
